...
LD_FLAGS = -X github.com/go-vela/vela-slack/version.Commit=${GITHUB_SHA} -X github.com/go-vela/vela-slack/version.Date=${BUILD_DATE} -X github.com/go-vela/vela-slack/version.Go=${GOLANG_VERSION} -X github.com/go-vela/vela-slack/version.Tag=${GITHUB_TAG}
...
build:
	@echo
	@echo "### Building release/vela-slack binary"
	GOOS=linux CGO_ENABLED=0 \
		go build -a \
		-ldflags '${LD_FLAGS}' \
		-o release/vela-slack \
		github.com/go-vela/vela-slack/cmd/vela-slack
...
build-static:
	@echo
	@echo "### Building static release/vela-slack binary"
	GOOS=linux CGO_ENABLED=0 \
		go build -a \
...
build-static-ci:
	@echo
	@echo "### Building CI static release/vela-slack binary"
	@go build -a \
		-ldflags '-s -w -extldflags "-static" ${LD_FLAGS}' \
		-o release/vela-slack \
		github.com/go-vela/vela-slack/cmd/vela-slack
...
docker-build:
	@echo
	@echo "### Building vela-slack:local image"
...
docker-test:
	@echo
	@echo "### Testing vela-slack:local image"
	@docker run --rm \
		vela-slack:local
...
docker-run:
	@echo
	@echo "### Executing vela-slack:local image"
	@docker run --rm \
